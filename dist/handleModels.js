...
const handleModels = (models, store, options) => {
  ...
  models.forEach(model => {
    ...
    if (init) {
      const prev = prevModels[namespace]; // Recall init only when init is changed

      const isSameInit = prev && prev.init && prev.init.toString() === init.toString();
